...
url2 = "https://leagueoflegends.fandom.com/wiki/Teamfight_Tactics:Champions"
...
champions = []
table = doc.at('table')
table.search('tr').drop(1).each_with_index do |tr,index|
  ...
  ability_info = ''
  1.times do
    ability_name = doc2.css('td.mw-collapsible')[index].children[1].text
    ability_info = doc2.css('td.mw-collapsible')[index].children[3].text.gsub("\n", "")
  end
...
File.open("scraped_data/test_scrape.json", 'w') { |file| file.write(json) }
